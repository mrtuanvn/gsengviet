...
- [x] Sửa lỗi "Error: No API found" trên giao diện Hugging Face Spaces (do Gradio 5 Server-Side Rendering (SSR) gây ra với ZeroGPU/spaces) bằng cách thêm `api_name="translate"` vào `translate_btn.click()` trong file `app.py` để định danh API cho frontend gọi một cách tường minh, đồng thời giữ nguyên `ssr_mode=False`.
